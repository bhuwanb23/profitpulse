"""
Test suite for Genome Orchestrator module
"""

import pytest
import numpy as np
import pandas as pd
from unittest.mock import Mock, patch

from src.data.preprocessing.client_genome.genome_orchestrator import (
    GenomeOrchestrator, process_client_genomes, analyze_genome_system
)


def test_genome_orchestrator_initialization():
    """Test Genome Orchestrator initialization"""
    orchestrator = GenomeOrchestrator()
    assert orchestrator is not None
    assert hasattr(orchestrator, 'genome_creator')
    assert hasattr(orchestrator, 'similarity_calculator')
    assert hasattr(orchestrator, 'clustering_engine')
    assert hasattr(orchestrator, 'comparison_tools')
    assert hasattr(orchestrator, 'genome_database')
    assert hasattr(orchestrator, 'processing_history')


def test_process_client_data():
    """Test processing client data"""
    orchestrator = GenomeOrchestrator()
    
    # Create mock client data
    client_data = pd.DataFrame({
        'client_id': ['client_1', 'client_2'],
        'revenue_std': [1000, 1500],
        'revenue_mean': [10000, 12000],
        'profit_margin_trend': [0.05, 0.03],
        'billing_accuracy': [0.95, 0.92],
        # Add other required features...
        'late_payment_rate': [0.02, 0.05],
        'cost_efficiency': [0.85, 0.8],
        'revenue_growth_rate': [0.1, 0.08],
        'contract_stability': [0.9, 0.85],
        'revenue_diversification': [0.7, 0.6],
        'forecast_accuracy': [0.8, 0.75],
        'cash_flow_ratio': [0.85, 0.8],
        'sla_compliance_rate': [0.95, 0.9],
        'avg_resolution_time': [24, 30],
        'target_resolution_time': [48, 48],
        'technician_productivity': [0.8, 0.75],
        'avg_quality_score': [4.2, 4.0],
        'resource_utilization_rate': [0.75, 0.7],
        'operational_cost_efficiency': [0.8, 0.75],
        'service_consistency': [0.9, 0.85],
        'automation_adoption_rate': [0.5, 0.4],
        'process_optimization_score': [0.7, 0.65],
        'scalability_score': [0.8, 0.75],
        'login_frequency': [15, 10],
        'feature_usage_depth': [0.7, 0.6],
        'support_requests_per_month': [2, 3],
        'communication_response_rate': [0.85, 0.8],
        'feedback_participation_rate': [0.4, 0.3],
        'training_completion_rate': [0.6, 0.5],
        'portal_engagement_score': [0.75, 0.7],
        'community_participation': [0.3, 0.25],
        'advocacy_score': [0.4, 0.35],
        'relationship_strength': [0.8, 0.75],
        'expansion_opportunity_score': [0.5, 0.45],
        'upsell_readiness': [0.6, 0.55],
        'market_position_strength': [0.7, 0.65],
        'innovation_adoption_rate': [0.5, 0.45],
        'partnership_potential': [0.4, 0.35],
        'cross_selling_opportunity': [0.6, 0.55],
        'revenue_growth_trajectory': [0.3, 0.25],
        'service_utilization_trend': [0.2, 0.15],
        'market_expansion_potential': [0.5, 0.45],
        'strategic_alignment': [0.7, 0.65],
        'churn_probability': [0.1, 0.15],
        'payment_delinquency_risk': [0.05, 0.08],
        'contract_expiration_risk': [0.2, 0.25],
        'service_quality_risk': [0.1, 0.15],
        'competitive_threat_level': [0.3, 0.35],
        'market_volatility_exposure': [0.2, 0.25],
        'dependency_risk': [0.15, 0.2],
        'compliance_risk': [0.1, 0.15],
        'operational_risk_score': [0.12, 0.18],
        'financial_stability_risk': [0.08, 0.12]
    })
    
    # Process client data
    client_genomes = orchestrator.process_client_data(client_data)
    
    # Check results
    assert len(client_genomes) == 2
    assert 'client_1' in client_genomes
    assert 'client_2' in client_genomes
    assert len(client_genomes['client_1']) == 50
    assert len(client_genomes['client_2']) == 50
    assert isinstance(client_genomes['client_1'], np.ndarray)
    assert isinstance(client_genomes['client_2'], np.ndarray)
    
    # Check that genomes are stored in database
    assert 'client_1' in orchestrator.genome_database
    assert 'client_2' in orchestrator.genome_database
    
    # Check processing history
    assert len(orchestrator.processing_history) == 1
    assert orchestrator.processing_history[0]['status'] == 'completed'


def test_analyze_client_similarity():
    """Test analyzing client similarity"""
    orchestrator = GenomeOrchestrator()
    
    # Create mock genome data
    genome1 = np.random.rand(50)
    genome2 = genome1.copy()
    
    orchestrator.genome_database = {
        'client_1': genome1,
        'client_2': genome2
    }
    
    # Analyze similarity
    similarity_analysis = orchestrator.analyze_client_similarity(
        'client_1', 'client_2'
    )
    
    # Check results
    assert isinstance(similarity_analysis, dict)
    assert 'clients' in similarity_analysis
    assert 'overall_similarities' in similarity_analysis
    assert similarity_analysis['clients']['client1'] == 'client_1'
    assert similarity_analysis['clients']['client2'] == 'client_2'


def test_cluster_clients():
    """Test clustering clients"""
    orchestrator = GenomeOrchestrator()
    
    # Create mock genome data
    orchestrator.genome_database = {
        'client_1': np.array([1] * 25 + [0] * 25),
        'client_2': np.array([1] * 25 + [0] * 25),
        'client_3': np.array([0] * 25 + [1] * 25),
        'client_4': np.array([0] * 25 + [1] * 25)
    }
    
    # Cluster clients
    cluster_assignments = orchestrator.cluster_clients(
        method='kmeans', n_clusters=2
    )
    
    # Check results
    assert len(cluster_assignments) == 4
    assert all(client_id in cluster_assignments for client_id in orchestrator.genome_database.keys())


def test_get_cluster_analysis():
    """Test getting cluster analysis"""
    orchestrator = GenomeOrchestrator()
    
    # Create mock genome data
    orchestrator.genome_database = {
        'client_1': np.array([1] * 25 + [0] * 25),
        'client_2': np.array([1] * 25 + [0] * 25),
        'client_3': np.array([0] * 25 + [1] * 25),
        'client_4': np.array([0] * 25 + [1] * 25)
    }
    
    cluster_assignments = {
        'client_1': 0,
        'client_2': 0,
        'client_3': 1,
        'client_4': 1
    }
    
    # Get cluster analysis
    cluster_analysis = orchestrator.get_cluster_analysis(cluster_assignments)
    
    # Check results
    assert isinstance(cluster_analysis, dict)
    assert len(cluster_analysis) == 2
    assert 0 in cluster_analysis
    assert 1 in cluster_analysis


def test_identify_anomalies():
    """Test identifying anomalies"""
    orchestrator = GenomeOrchestrator()
    
    # Create mock genome data with outliers
    orchestrator.genome_database = {
        'client_1': np.array([0.5] * 50),
        'client_2': np.array([0.5] * 50),
        'client_3': np.array([0.5] * 50),
        'client_4': np.array([0.5] * 49 + [0.99])
    }
    
    # Identify anomalies
    anomalies = orchestrator.identify_anomalies(threshold=2.0)
    
    # Check results
    assert isinstance(anomalies, dict)


def test_find_similar_clients():
    """Test finding similar clients"""
    orchestrator = GenomeOrchestrator()
    
    # Create mock genome data
    target_genome = np.array([1] * 25 + [0] * 25)
    orchestrator.genome_database = {
        'target_client': target_genome,
        'similar_client': np.array([1] * 25 + [0] * 25),
        'different_client': np.array([0] * 25 + [1] * 25)
    }
    
    # Find similar clients
    similar_clients = orchestrator.find_similar_clients(
        'target_client', top_k=2
    )
    
    # Check results
    assert isinstance(similar_clients, list)
    assert len(similar_clients) == 2
    # The first result might be the target client itself (with similarity 1.0)
    # or one of the other clients, depending on implementation details
    client_ids = [client[0] for client in similar_clients]
    assert 'target_client' in client_ids or 'similar_client' in client_ids


def test_generate_client_profile():
    """Test generating client profile"""
    orchestrator = GenomeOrchestrator()
    
    # Create mock genome data
    genome = np.random.rand(50)
    orchestrator.genome_database = {
        'test_client': genome
    }
    
    # Generate client profile
    profile = orchestrator.generate_client_profile('test_client')
    
    # Check results
    assert isinstance(profile, dict)
    assert 'client_id' in profile
    assert 'genome_timestamp' in profile
    assert 'categories' in profile
    assert 'overall_assessment' in profile
    assert profile['client_id'] == 'test_client'


def test_convenience_functions():
    """Test convenience functions"""
    # Test process_client_genomes
    client_data = pd.DataFrame({
        'client_id': ['client_1'],
        'revenue_std': [1000],
        'revenue_mean': [10000],
        'profit_margin_trend': [0.05],
        'billing_accuracy': [0.95],
        'late_payment_rate': [0.02],
        # Add minimal set of features for testing
    })
    
    client_genomes = process_client_genomes(client_data)
    assert len(client_genomes) == 1
    assert 'client_1' in client_genomes
    
    # Test analyze_genome_system
    analysis_results = analyze_genome_system(client_data)
    assert isinstance(analysis_results, dict)
    assert 'client_genomes' in analysis_results


if __name__ == '__main__':
    pytest.main([__file__])